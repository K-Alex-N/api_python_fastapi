...
def test_checkboxes_selection(checkbox_page, check_node, expected_present, expected_absent):
    checkbox_page.select_checkbox(check_node)
    selected = checkbox_page.get_selected_values()
    for value in expected_present:
        assert value in selected, f"Expected '{value}' to be selected. Output: {selected}"
    for value in expected_absent:
        assert value not in selected, f"Did not expect '{value}' to be selected. Output: {selected}"
