...
class TextInput(BaseElement):
    ...
    async def fill(self, value: str) -> None:
        with allure.step(f"Fill input with value: {value}"):
...
class TextElement(BaseElement):
    ...
    async def should_have_text(self, expected_text: str) -> None:
        with allure.step(f"Text should have {expected_text}"):
            await self.should_be_visible()
            await expect(self.locator).to_have_text(expected_text)
